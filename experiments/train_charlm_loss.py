...
def generate_text(
    ...
    model.eval()
    idx = torch.tensor(
        [[stoi.get(c, 0) for c in prompt]], device=device
    )  # shape [1, seq_len]
    ...
    with torch.no_grad():
        for _ in range(max_new_tokens):
            ...
            next_idx = topk_idx[0, torch.multinomial(topk_probs, 1)]  # shape []

            next_char = itos[next_idx.item()]
            generated.append(next_char)

            # Make next_idx 2D to match idx for concatenation
            next_idx = next_idx.view(1, 1)  # ✅ shape [1,1]
...
def train(args):
    ...
    # Auto split into train/val if no val_data provided
    ...
    while True:
        for x, y in train_loader:
            ...
            # Save checkpoint + generate sample (every 1000 steps)
            if global_step > 0 and global_step % save_interval == 0:
                ckpt_path = f"checkpoints/loss_step{global_step}.pt"
                torch.save(
                    {
...
if __name__ == "__main__":
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Optional validation file. If not given, dataset will be split automatically.",
    )
