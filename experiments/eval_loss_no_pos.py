...
class CharTransformerLM(nn.Module):
    def __init__(self, vocab_size, d_model, n_heads=1, n_layers=2, d_ff=None):
        super().__init__()
        if d_ff is None:
            d_ff = 4 * d_model
    ...
    def forward(self, x):
        h = self.token_emb(x)  # ❌ No positional encoding
...
def load_model(ckpt_path, device):
    ...
    model = CharTransformerLM(
        ...
        n_heads=args.get("n_heads", 1),
        n_layers=args.get("n_layers", 2),
        d_ff=4 * args.get("d_model", 192),
    )
